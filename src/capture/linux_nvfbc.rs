...
unsafe fn try_init_cuda() -> Result<CudaContextGuard, String> {
    ...
    let cu_device_get: unsafe extern "C" fn(*mut i32, i32) -> i32 = std::mem::transmute(cu_device_get_ptr);
    ...
    let cu_ctx_create: unsafe extern "C" fn(*mut *mut std::ffi::c_void, u32, i32) -> i32 = std::mem::transmute(cu_ctx_create_ptr);
    ...
    let cu_ctx_destroy: unsafe extern "C" fn(*mut std::ffi::c_void) -> i32 = std::mem::transmute(cu_ctx_destroy_ptr);
...
unsafe fn custom_start_nvfbc(
    ...
    params.bWithCursor = if with_cursor { nvfbc_sys::_NVFBC_BOOL_NVFBC_TRUE } else { nvfbc_sys::_NVFBC_BOOL_NVFBC_FALSE };
    params.frameSize = nvfbc_sys::NVFBC_SIZE { w: 0, h: 0 };
    params.eTrackingType = nvfbc_sys::NVFBC_TRACKING_TYPE_NVFBC_TRACKING_DEFAULT;
    params.dwSamplingRateMs = (1000 / fps) as u32;
...
fn init_capturer() -> Result<(ActiveCapturer, Option<CudaContextGuard>), String> {
    ...
    let (xlib_handle, x_display, set_handler_fn) = unsafe {
        let xlib = libc::dlopen(b"libX11.so.6\0".as_ptr() as *const libc::c_char, libc::RTLD_LAZY);
        if xlib.is_null() {
            ...
            let display = if !x_open.is_null() {
                let open_fn: unsafe extern "C" fn(*const libc::c_char) -> *mut libc::c_void = std::mem::transmute(x_open);
    ...
    let prev_handler: Option<unsafe extern "C" fn(*mut libc::c_void, *mut libc::c_void) -> libc::c_int> = unsafe {
        ...
        if !display.is_null() && !xlib.is_null() {
            unsafe {
                ...
                if !x_sync_sym.is_null() {
                    let sync_fn: unsafe extern "C" fn(*mut libc::c_void, libc::c_int) -> libc::c_int = std::mem::transmute(x_sync_sym);
    ...
    unsafe {
        ...
        if !x_display.is_null() && !xlib_handle.is_null() {
            let x_close = libc::dlsym(xlib_handle, b"XCloseDisplay\0".as_ptr() as *const libc::c_char);
            if !x_close.is_null() {
                let close_fn: unsafe extern "C" fn(*mut libc::c_void) -> libc::c_int = std::mem::transmute(x_close);
                close_fn(x_display);
            }
        }
...
fn init_capturer_inner(
    ...
    match unsafe { try_init_cuda() } {
        Ok(guard) => {
            ...
            log::info!("CUDA initialized successfully. Attempting to initialize NvFBC CUDA Capturer...");
            match CudaCapturer::new() {
                Ok(c) => {
                    // XSync to flush any X errors from CudaCapturer::new()
                    x_sync(x_display, xlib_handle);
                    if X11_ERROR_OCCURRED.load(AtomicOrdering::SeqCst) {
                        log::warn!("X11/GLX error occurred during NvFBC CUDA Capturer init — cleaning up");
        ...
        Err(e) => {
            log::warn!("CUDA context initialization failed: {}. Falling back to NvFBC System Capturer", e);
        }
    }
    ...
    match SystemCapturer::new() {
        ...
        Err(e) => Err(format!("Failed to initialize NvFBC System Capturer: {:?}", e)),
    }
}
...
impl NvfbcCapture {
    ...
    pub fn new() -> Result<Self, MediaError> {
                ...
                loop {
                    ...
                    while let Ok(cmd) = cmd_rx.try_recv() {
                        match cmd {
                            ...
                            NvfbcCommand::Start { config, frame_tx, reply } => {
        ...
        cmd_tx.send(NvfbcCommand::GetStatus { reply: status_tx })
            .map_err(|_| MediaError::CaptureError("Failed to communicate with NvFBC thread".into()))?;

        let status = status_rx.recv()
            .map_err(|_| MediaError::CaptureError("NvFBC thread query failed".into()))?
            .map_err(|e_str| MediaError::CaptureError(format!("Failed to query status: {}", e_str)))?;
...
impl Drop for NvfbcCapture {
    fn drop(&mut self) {
        ...
        if self.cmd_tx.send(NvfbcCommand::Stop { reply: reply_tx }).is_ok() {
            let _ = reply_rx.recv();
        }
...
impl ScreenCapture for NvfbcCapture {
    ...
    async fn start(&mut self, _display_id: &str, config: &StreamConfig) -> Result<(), MediaError> {
        ...
        let (reply_tx, reply_rx) = std::sync::mpsc::channel();
        self.cmd_tx.send(NvfbcCommand::Start {
            config: config.clone(),
            frame_tx,
            reply: reply_tx,
        })
        ...
        reply_rx.recv()
            ...
            .map_err(|e| MediaError::CaptureError(format!("Failed to start NvFBC session: {}", e)))?;
